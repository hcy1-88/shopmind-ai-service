...
class GraphFactory:
    ...
    3. 初始化 ShopmindAgentGraph 并持有子图引用
    """
    ...
    def build_all(cls, checkpointer: BaseCheckpointSaver) -> ShopmindAgentGraph:
        ...
        # 3. 初始化父图
        main_graph = cls._build_main_graph(checkpointer, shopping_subgraph, chitchat_agent)

        logger.info("GraphFactory 所有图构建完成!")
        return main_graph

    @classmethod
    def _build_shopping_subgraph(cls, checkpointer: BaseCheckpointSaver) -> ShoppingSubgraph:
        """
        构建 Shopping 子图
        ...
        Returns:
            ShoppingSubgraph 单例
        """
        shopping_subgraph = ShoppingSubgraph.get_instance()
    ...
    def _build_main_graph(
        ...
        shopping_subgraph: ShoppingSubgraph,
        ...
        main_graph.init_graph(checkpointer, shopping_subgraph, chitchat_agent)
